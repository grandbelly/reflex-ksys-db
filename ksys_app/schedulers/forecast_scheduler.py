...
class ForecastScheduler:
    ...
    def __init__(self):
        ...
        # Forecast horizons: 1h, 2h, 6h, 12h, 24h as per Phase 2 documentation
        # Can be customized based on operational needs
        self.horizons = [60, 120, 360, 720, 1440]  # minutes
    ...
    async def generate_predictions(
        ...
        try:
            ...
            if model_type in ['AUTO_ARIMA', 'PROPHET']:
                # Model plugins have async predict() wrapper - await directly
                forecast_df = await model.predict(len(horizons))

                # Extract predictions and confidence intervals
                # Column names vary by model type:
                # AUTO_ARIMA: 'AutoARIMA', 'AutoARIMA-lo-95', 'AutoARIMA-hi-95'
                # PROPHET: 'yhat', 'yhat_lower', 'yhat_upper'
                if model_type == 'AUTO_ARIMA':
                    ...
                    # Prophet uses different column names
                    # Log available columns for debugging
                    logger.debug(f"Prophet forecast columns: {forecast_df.columns.tolist()}")

                    # Try to find prediction column (yhat, y, or second numeric column)
                    if 'yhat' in forecast_df.columns:
                        predictions = forecast_df['yhat'].tolist()
                    elif 'y' in forecast_df.columns:
                        predictions = forecast_df['y'].tolist()
                    else:
                        # Find first numeric column that's not 'ds' (timestamp)
                        numeric_cols = forecast_df.select_dtypes(include=['float64', 'float32', 'int64', 'int32']).columns
                        if len(numeric_cols) > 0:
                            predictions = forecast_df[numeric_cols[0]].tolist()
                        else:
                            logger.error(f"❌ No numeric prediction column found in Prophet output: {forecast_df.columns.tolist()}")
                            predictions = [last_value] * len(horizons)

                    ci_lower = forecast_df['yhat_lower'].tolist() if 'yhat_lower' in forecast_df.columns else predictions
                    ci_upper = forecast_df['yhat_upper'].tolist() if 'yhat_upper' in forecast_df.columns else predictions
    ...
    async def save_predictions(
        ...
        async with get_async_session() as session:
            insert_sql = text("""
                ...
                ON CONFLICT (target_time, model_id, tag_name, horizon_minutes) DO NOTHING
            """)
