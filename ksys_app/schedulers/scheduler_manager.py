...
class SchedulerManager:
    ...
    async def start_all(self):
        ...
        try:
            ...
            # Start ForecastScheduler (every 5 minutes, at x:00)
            async def start_forecast_with_delay():
                await asyncio.sleep(forecast_delay)
                await forecast_scheduler.start(interval_seconds=300)  # 5 minutes (as per docs)

            forecast_task = asyncio.create_task(start_forecast_with_delay())
            self.tasks.append(forecast_task)
            logger.info("✅ ForecastScheduler task created (5-minute interval, starts at x:00)")
            ...
            # Start PerformanceAggregator (every 1 hour, at x:03)
            async def start_aggregator_with_delay():
                await asyncio.sleep(aggregator_delay)
                await performance_aggregator.start(interval_seconds=3600)  # 1 hour (as per docs)

            aggregator_task = asyncio.create_task(start_aggregator_with_delay())
            self.tasks.append(aggregator_task)
            logger.info("✅ PerformanceAggregator task created (1-hour interval, starts at x:03)")
