...
class AlarmService:
    ...
    async def get_rule_based_alarms(
        ...
        Args:
            ...
            search_query: Search query for tag_name, message, or cause
        ...
        try:
            ...
            # Match all RULE-based scenarios: use ai_generated = false to distinguish from AI-generated alarms
            where_clauses = [
                "ai_generated = false",  # Rule-based alarms (not AI-generated)
                "triggered_at >= NOW() - :hours * INTERVAL '1 hour'"
            ]

            # Add search query filter (tag_name, message, cause)
            if search_query:
                # Search in scenario_id (tag_name), message, and cause
                search_pattern = f"%%{search_query}%%"
                where_clauses.append(
                    f"(scenario_id ILIKE :search_pattern OR message ILIKE :search_pattern OR sensor_data->>'cause' ILIKE :search_pattern)"
                )
            ...
            if search_query:
                query_params["search_pattern"] = f"%{search_query}%"

            count_result = await self.session.execute(count_q, query_params)
            ...
            result = await self.session.execute(q, query_params)
            ...
            for row in rows:
                ...
                alarms.append({
                    ...
                    "value_formatted": f"{float(row.get('value', 0)):.2f}" if row["value"] else "0.00",
                    "unit": row["unit"] or "",
                    "threshold_low": float(row.get("threshold_low", 0)) if row.get("threshold_low") else None,
                    "threshold_high": float(row.get("threshold_high", 0)) if row.get("threshold_high") else None,
                    "cause": row["cause"] or "",
