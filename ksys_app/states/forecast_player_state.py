...
class ForecastPlayerState(BaseState):
    ...
    cached_r2: Optional[float] = None  # ✅ FIX: Real R² score (not approximation)
    ...
    def accuracy_metrics(self) -> Dict:
        ...
        if self.cached_mape is not None or self.cached_rmse is not None or self.cached_mae is not None:
            return {
                ...
                "r2": round(self.cached_r2, 4) if self.cached_r2 is not None else None,  # ✅ FIX: Use real R² score
    ...
    def r2_display(self) -> str:
        """Display R² value - use real statistical R² score"""
        # Use cached R² if available
        if self.cached_r2 is not None:
            return f"{self.cached_r2:.3f}"
    ...
    @rx.var
    def chart_data_length(self) -> int:
        """Get number of points in rolling window data (chart_data alias)"""
        return len(self.rolling_window_data)
    ...
    # ✅ REMOVED: scheduler_running Docker ps check that ran on every render
    # This caused performance issues and permission errors on RPI
    # Scheduler status is now assumed to be running (static True)
    # For admin monitoring, use dedicated health check page instead
    @rx.var
    def scheduler_running(self) -> bool:
        """Scheduler running status - assumed true (avoid Docker ps overhead)"""
        return True  # Static assumption - use health check page for actual monitoring
    ...
    async def load_deployed_models(self):
        ...
        try:
            ...
            # ✅ NOTE: start_countdown_monitor is already called via on_load in page decorator
            # No need to manually spawn it here to avoid TypeError: EventSpec cannot be awaited
            # See: ksys_app/pages/forecast_player.py on_load configuration
    ...
    async def _load_predictions_internal(self):
        ...
        try:
            ...
            async with get_async_session() as session:
                ...
                mae_values = []
                squared_errors = []
                actual_values = []  # ✅ For R² calculation
                predicted_values = []  # ✅ For R² calculation
                ...
                for point in rolling_data:
                    ...
                    if actual is not None and pred is not None and actual != 0:
                        ...
                        actual_values.append(actual)  # ✅ Store for R²
                        predicted_values.append(pred)  # ✅ Store for R²
                ...
                # ✅ FIX: Calculate R² score using proper statistical formula
                # R² = 1 - (SS_res / SS_tot)
                # where SS_res = Σ(y_actual - y_predicted)²
                #       SS_tot = Σ(y_actual - y_mean)²
                cached_r2 = None
                if len(actual_values) >= 2:  # Need at least 2 points for meaningful R²
                    y_mean = sum(actual_values) / len(actual_values)
                    ss_res = sum((a - p) ** 2 for a, p in zip(actual_values, predicted_values))
                    ss_tot = sum((a - y_mean) ** 2 for a in actual_values)
                    if ss_tot > 0:  # Avoid division by zero
                        cached_r2 = 1 - (ss_res / ss_tot)
                        # Clamp R² to [-∞, 1] range (negative R² means worse than mean)
                        # But for display, we'll show it as-is
                ...
                r2_str = f"{cached_r2:.4f}" if cached_r2 is not None else "N/A"
                console.debug(f"📈 [CALCULATED] MAPE={mape_str}%, MAE={mae_str}, RMSE={rmse_str}, R²={r2_str}")
            ...
            async with self:
                ...
                self.cached_r2 = cached_r2  # ✅ FIX: Store real R² score
    ...
    async def start_countdown_monitor(self):
        ...
        # Exit condition: Check is_streaming flag to prevent infinite loops
        poll_counter = 0
        while self.is_streaming:  # ✅ Exit when user leaves page or stops streaming
            await asyncio.sleep(30)  # ✅ Update every 30 seconds (minimize UI thrashing)
            poll_counter += 1

            # ✅ Check again after sleep (user may have left page)
            if not self.is_streaming:
                console.info("⏹️ [COUNTDOWN] Stopping countdown monitor (is_streaming=False)")
                break
