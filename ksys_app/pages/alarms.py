...
def history_alarms_view() -> rx.Component:
    ...
    def history_row(alarm: Dict) -> rx.Component:
        return rx.table.row(
            ...
            _hover={"bg": "rgba(59, 130, 246, 0.05)", "cursor": "pointer"})
    return rx.cond(AlarmsState.total_count > 0,
        rx.vstack(
            rx.table.root(
                rx.table.header(rx.table.row(
                    rx.table.column_header_cell("상태", width="100px", style={"backgroundColor": "#f9fafb !important", "color": "#374151 !important"}),
            ...
            pagination(
                ...
                total_items=AlarmsState.total_count,  # Use total_count directly (server-side value)
                page_size=AlarmsState.page_size,
                # page_info=AlarmsState.page_info,  # Don't pass - let pagination compute it from total_items
